...
% read data from Excelsheet
  data=xlsread('NewProteaseAnalysisV5.xls', 3, 'A3:DP999');
% save data
% load('data.mat')

nameVirus={'CoV1','CoV2','Bat','BA5','XBB','Omicron','Murine','Mers','OC43','F229E','NL63','HKU1','RaTG13','Pangolin','MurineHep','PorcRes','PorcGas','Canine','Swine','Feline','Dog','Bovine','Rabbit','Erinaceus'};
...
for z = 0.1:0.01:10
    ...
    record19(i)= log(Langlands(Swine,z));
    record20(i)= log(Langlands(Feline,z));
    record21(i)= log(Langlands(Dog,z));
    record22(i)= log(Langlands(Bovine,z));
    record23(i)= log(Langlands(Rabbit,z));
    record24(i)= log(Langlands(Erinaceus,z));
    % next virus 
    i=i+1;
end

% Calulate the distance among each others
record=(record1+record2+record3+record4+record5++record6+record7+record8+record9+record10+record11+record12+record13+record14++record15+record16+record17+record18+record19+record20+record21+record22+record23+record24)/24;
semi(1)=(mean((record1-record).^(1/24)))^24;
semi(2)=(mean((record2-record).^(1/24)))^24;
semi(3)=(mean((record3-record).^(1/24)))^24;
semi(4)=(mean((record4-record).^(1/24)))^24;
semi(5)=(mean((record5-record).^(1/24)))^24;
semi(6)=(mean((record6-record).^(1/24)))^24;
semi(7)=(mean((record7-record).^(1/24)))^24;
semi(8)=(mean((record8-record).^(1/24)))^24;
semi(9)=(mean((record9-record).^(1/24)))^24;
semi(10)=(mean((record10-record).^(1/24)))^24;
semi(11)=(mean((record11-record).^(1/24)))^24;
semi(12)=(mean((record12-record).^(1/24)))^24;
semi(13)=(mean((record13-record).^(1/24)))^24;
semi(14)=(mean((record14-record).^(1/24)))^24;
semi(15)=(mean((record15-record).^(1/24)))^24;
semi(16)=(mean((record16-record).^(1/24)))^24;
semi(17)=(mean((record17-record).^(1/24)))^24;
semi(18)=(mean((record18-record).^(1/24)))^24;
semi(19)=(mean((record19-record).^(1/24)))^24;
semi(20)=(mean((record20-record).^(1/24)))^24;
semi(21)=(mean((record21-record).^(1/24)))^24;
semi(22)=(mean((record22-record).^(1/24)))^24;
semi(23)=(mean((record23-record).^(1/24)))^24;
semi(24)=(mean((record24-record).^(1/24)))^24;

% The real of spectrum
figure(1)
loglog(recordZ,real(record1),recordZ,real(record2),'--',recordZ,real(record3),recordZ,real(record4),'--',recordZ,real(record5),recordZ,real(record6),'--',recordZ,real(record7),recordZ,real(record8),'--',recordZ,real(record9),recordZ,real(record10),'--',recordZ,real(record11),recordZ,real(record12),'--',recordZ,real(record13),recordZ,real(record14),'--',recordZ,real(record15),'-.',recordZ,real(record16),'-.',recordZ,real(record17),'-.',recordZ,real(record18),'-.',recordZ,real(record19),'-.',recordZ,real(record20),'-.',recordZ,real(record21),'-.',recordZ,real(record22),':',recordZ,real(record23),':',recordZ,real(record24),':','LineWidth',1);
legend(nameVirus);
...
figure(2)
loglog(recordZ,imag(record1),recordZ,imag(record2),'--',recordZ,imag(record3),recordZ,imag(record4),'--',recordZ,imag(record5),recordZ,imag(record6),'--',recordZ,imag(record7),recordZ,imag(record8),'--',recordZ,imag(record9),recordZ,imag(record10),'--',recordZ,imag(record11),recordZ,imag(record12),'--',recordZ,imag(record13),recordZ,imag(record14),'--',recordZ,imag(record15),'-.',recordZ,imag(record16),'-.',recordZ,imag(record17),'-.',recordZ,imag(record18),'-.',recordZ,imag(record19),'-.',recordZ,imag(record20),'-.',recordZ,imag(record21),'-.',recordZ,imag(record22),':',recordZ,imag(record23),':',recordZ,imag(record24),':','LineWidth',1);
legend(nameVirus);
